...
def call(args) {
  ...
  docker.image('923402097046.dkr.ecr.eu-central-1.amazonaws.com/buildtools/serverless-deploy').inside("-e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI=${env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI}") {
    ...
    sh "serverless deploy $args --provider aws --key $key --secret $secretKey"
  }
}
