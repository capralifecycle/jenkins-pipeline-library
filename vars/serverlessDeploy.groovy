...
def call(deployIamRole, serverlessArgs) {
  ...
  docker.image('923402097046.dkr.ecr.eu-central-1.amazonaws.com/buildtools/tool/serverless').inside("-e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI=${env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI}") {
    sh '''
      #!/bin/bash
      set +x
